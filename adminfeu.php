...
if(isset($_POST['updateSubmit'])){
            ...
            $sql = "UPDATE `admin_featured` SET `Product Name`='$product_name',`Price`='$price',`Image`='$image' WHERE `Id` = $id";

            $result = mysqli_query($con, $sql);
            if($result){
                header("location: adminfeatured.php");
            }else{
                echo "Not Updated";
            }
}
...
<?php
    if(isset($_POST['submit'])){
        $id = $_POST['id'];
        $sql = "SELECT * FROM `admin_featured` WHERE Id = '$id'";
        $result = mysqli_query($con, $sql);
        if($result){
